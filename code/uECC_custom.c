...
/*** Customized functions code BEGIN **********************************************/
static int uECC_sign_with_k_internal_custom(const uint8_t *private_key,
                            const uint8_t *message_hash,
                            unsigned hash_size,
                            uECC_word_t *k,
                            uint8_t *signature,
	...
	uECC_word_t halfCurveOrder[uECC_MAX_WORDS];
	uint32_t v_tmp = 0;
	cmpresult_t sig_s = 0;
	uECC_word_t tmp[uECC_MAX_WORDS];
    uECC_word_t s[uECC_MAX_WORDS];
    ...
    /* Customized code for computation of v */
    /* This case is for BIG ENDIAN
     */
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    *v = 0;
    if (uECC_vli_cmp(curve->n, p, num_n_words) != 1){ /* r.x is greater than curve order (very VERY rare) */
    	*v = 2;
    }
	if (p[15] % 2) { /* r.y is odd */
		v_tmp = *v;
		v_tmp++;
		*v = v_tmp;
	}
//	uECC_vli_set(halfCurveOrder, curve->n, uECC_MAX_WORDS);
//	uECC_vli_rshift1(halfCurveOrder, uECC_MAX_WORDS);
//	sig_s = uECC_vli_cmp(halfCurveOrder, s, uECC_MAX_WORDS);
//    if (sig_s != 1){ /* s > (curve.N)/2 */
//    	v_tmp = *v;
//    	v_tmp ^= 1; // perform a flip on v
//    	*v = v_tmp;
//    }
#else
    /* Customized code for computation of v */
    /* This case is for LITTLE ENDIAN
     */
    *v = 0;
	if ((p[8] & 0x00000001) == 0x00000001) { /* r.y is odd */
		v_tmp = *v;
		v_tmp++;
		*v = v_tmp;
	}
	uECC_vli_set(halfCurveOrder, curve->n, num_n_words);
	uECC_vli_rshift1(halfCurveOrder, num_n_words);
	sig_s = uECC_vli_cmp(halfCurveOrder, s, num_n_words);
    if (sig_s != 1) { /* s > (curve.N)/2 */
    	/* Obtain -s and change v */
	...
	uECC_vli_set(k_final, k, num_words);
...
int uECC_sign_custom(const uint8_t *private_key,
              const uint8_t *message_hash,
              unsigned hash_size,
              uint8_t *signature,
			  uint32_t *v,
    ...
    for (tries = 0; tries < uECC_RNG_MAX_TRIES; ++tries) {
        ...
        if (uECC_sign_with_k_internal_custom(private_key, message_hash, hash_size, k, signature, v, k_final, curve)) {
            return 1;
        }
    }
    return 0;
...
int uECC_sign_deterministic_custom(const uint8_t *private_key,
              const uint8_t *message_hash,
              unsigned hash_size,
			  uECC_word_t *k,
              uint8_t *signature,
